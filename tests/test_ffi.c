...
int main(void) {
    ...
    CHECK(sizeof(odg_ffi_abi_info) == 112u);
    ...
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_CAMERA_PROFILE) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_REMOTE_VIEW) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_HYDRATION) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_FLUID_REGISTRY) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_SAVE_SCHEMA_QUERY) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_FAUNA_NESTING_REGISTRY) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_CONSTRUCTION_BLOCKS) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_CONSTRUCTION_SHAPES) != 0u);
    CHECK((abi.feature_bits & ODG_FFI_FEATURE_CONSTRUCTION_DURABILITY) != 0u);
    CHECK(abi.construction_snapshot_size == sizeof(odg_construction_snapshot));
    CHECK(abi.food_definition_size == sizeof(odg_food_definition));
    CHECK(abi.fauna_species_definition_size == sizeof(odg_fauna_species_definition));
    CHECK(abi.fauna_snapshot_size == sizeof(odg_fauna_snapshot));
    CHECK(abi.fluid_definition_size == sizeof(odg_fluid_definition));
    CHECK(abi.fluid_container_definition_size == sizeof(odg_fluid_container_definition));
    CHECK(abi.fauna_nesting_definition_size == sizeof(odg_fauna_nesting_definition));
